...
const BlogCommentModal = ({ isOpen, onClose, onSubmit, Id }) => {
  const {
    register,
    handleSubmit,
    setValue,
  ...
  const getSingleData = async () => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ADMIN_URL}blogcomment/getblogcommentbyid/${Id}`,
        {
          headers: {
      ...
      >
        ...
        <div className="flex items-center justify-between mb-2">
          <h6 className="text-lg font-semibold mx-auto">
            {isEditMode ? "Edit " : "Add "} blog comment
          </h6>
          <button
            type="button"
            onClick={onClose}
