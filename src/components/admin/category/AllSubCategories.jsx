...
const AllSubCategories = () => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ADMIN_URL}categories/getall?PageNumber=${
          page - 1
        }&SearchText=&SortBy=Name&SortDirection=asc&PageSize=1000`,
  ...
  const handleSubCategories = async (data) => {
    ...
    const endpoint = isEditing
      ? `${process.env.NEXT_PUBLIC_API_ADMIN_URL}subcategories/update/${selectedId}`
      : `${process.env.NEXT_PUBLIC_API_ADMIN_URL}subcategories/create`;
                ...
                {allData.map((item, inx) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-3 flex items-center gap-2">
                      <img
                        src={`${process.env.NEXT_PUBLIC_API_ADMIN_URL}files/subcategories/${item.imageUrl}`}
                        alt={item.name}
                    ...
                    <td className="py-4 px-2 font-medium">
                      <div className=" flex items-center gap-2">
                        ...
                        <DeleteButton
                          endpoint={`subcategories/delete/${item?.id}`}
                          type="sub category"
                          onComplete={(status) => {
                            if (status) {
                              setAllData((prev) =>
