...
const AllServicesTable = ({ serviceStatus }) => {
  ...
  const getServices = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_ADMIN_URL}service/getall?PageNumber=${
          page - 1
        }&SearchText=&SortBy=Title&SortDirection=asc&PageSize=${pageSize}&StatusType=${serviceStatus}`,
              ...
              <tbody className="text-[13px]">
                {allData.map((item, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-3 font-medium">{item.price}</td>
                    ...
                    <td className="py-4 px-2 font-medium">
                      <div className=" flex items-center gap-2">
                        ...
                        <DeleteButton
                          endpoint={`service/delete/${item?.id}`}
                          type="service"
                          onComplete={(status) => {
                            if (status) {
                              setAllData((prev) =>
